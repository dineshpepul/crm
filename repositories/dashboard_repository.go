...
func (r *gormDashboardRepository) GetDashboardSummary() (map[string]interface{}, error) {
	summary := make(map[string]interface{})

	// Get total leads
	var totalLeads int64
	if err := r.db.Model(&models.Lead{}).Count(&totalLeads).Error; err != nil {
		return nil, err
	}
	...
	if err := r.db.Model(&models.Lead{}).Where("DATE(created_at) = ?", today).Count(&newLeadsToday).Error; err != nil {
		return nil, err
	}
	...
	if err := r.db.Model(&models.Lead{}).Where("status = ?", "qualified").Count(&qualifiedLeads).Error; err != nil {
		return nil, err
	}
	...
	if err := r.db.Model(&models.Deal{}).Count(&totalDeals).Error; err != nil {
		return nil, err
	}
	summary["total_deals"] = totalDeals

	if err := r.db.Model(&models.Deal{}).Where("stage = ?", "won").Count(&dealsWon).Error; err != nil {
		return nil, err
	}
	summary["deals_won"] = dealsWon

	if err := r.db.Model(&models.Deal{}).Where("stage = ?", "lost").Count(&dealsLost).Error; err != nil {
		return nil, err
	}
	...
	if err := r.db.Model(&models.Deal{}).Where("stage = ?", "won").Select("COALESCE(SUM(amount), 0)").Row().Scan(&totalRevenue); err != nil {
		return nil, err
	}
	summary["total_revenue"] = totalRevenue

	var forecastedRevenue float64
	if err := r.db.Model(&models.Deal{}).Where("stage NOT IN (?, ?)", "won", "lost").
...
func (r *gormDashboardRepository) GetLeadsBySource() ([]map[string]interface{}, error) {
	var results []struct {
		Source string
		Count  int
	}
...
func (r *gormDashboardRepository) GetLeadsByStatus() ([]map[string]interface{}, error) {
	var results []struct {
		Status string
		Count  int
	}
...
func (r *gormDashboardRepository) GetRevenueByMonth(year int) ([]map[string]interface{}, error) {
	var results []struct {
		Month   int
		Revenue float64
	}
	...
	if err := r.db.Model(&models.Deal{}).
		...
		Where("YEAR(created_at) = ? AND stage = ?", year, "won").
...
func (r *gormDashboardRepository) GetSalesForecast(months int) ([]map[string]interface{}, error) {
	var results []struct {
		Year             int
		Month            int
	...
	if err := r.db.Model(&models.Deal{}).
		...
		Where("stage NOT IN (?, ?) AND expected_close_date IS NOT NULL AND expected_close_date <= DATE_ADD(CURDATE(), INTERVAL ? MONTH)",
			"won", "lost", months).
